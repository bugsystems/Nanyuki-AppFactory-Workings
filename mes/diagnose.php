...
<body>
	<div id="templatemo_container">
        ...
        <div id="templatemo_content_area">
		 <?php
		  include "connect.php";
		  $sql=mysqli_query($con,"SELECT DISTINCT symptom FROM diseases ORDER BY id DESC");
		  $sql1=mysqli_query($con,"SELECT DISTINCT symptom FROM diseases ORDER BY id DESC");
		  $sql2=mysqli_query($con,"SELECT DISTINCT symptom FROM diseases ORDER BY id DESC");
		  $sql3=mysqli_query($con,"SELECT DISTINCT symptom FROM diseases ORDER BY id DESC");
		  ?>
		  <form action="diagnose_p.php" method="post">
		  <pre>
		  First Symptom   <select name="symptom1">
		  <?php while($r=mysqli_fetch_array($sql)){ ?>
		  <option><?php echo $r['symptom'] ?></option>
		  <?php }?>
		  </select>
		  
		  Second Symptom  <select name="symptom2">
		  <?php while($r1=mysqli_fetch_array($sql1)){ ?>
		  <option><?php echo $r1['symptom'] ?></option>
		  <?php }?>
		  </select>
		  
		  Third Symptom   <select name="symptom3">
		  <?php while($r1=mysqli_fetch_array($sql2)){ ?>
		  <option><?php echo $r1['symptom'] ?></option>
		  <?php }?>		  
		  </select>
		  
		  Fourth Symptom  <select name="symptom4">
		  <?php while($r1=mysqli_fetch_array($sql3)){ ?>
		  <option><?php echo $r1['symptom'] ?></option>
		  <?php }?>
		  </select>	
        ...
        <!-- InstanceEndEditable -->
        <div class="cleaner"></div>
        <div id="templatemo_footer">Copyright © 2016 <a href="#">The Bug Systems</a><a href="http://www.templatemo.com/page/1" target="_parent"></a> </div>
