...
$sql=mysqli_query($con,"SELECT * FROM ache WHERE ache='$ac'");

echo "Please take the following Drug:<br>";
echo "<ul>";
while($row=mysqli_fetch_array($sql))
{
echo "<li>".$row['diagnosis']."</li>";
}
echo "</ul>";
...
?>
		 <?php
		  include "connect.php";
		  $sql=mysqli_query($con,"SELECT DISTINCT ache FROM ache ORDER BY id DESC");
		 
		  ?>
		  
		  <form action="ache_p.php" method="post">
		  <pre>
		  Ailment   <select name="symptom1">
		  <?php while($r=mysqli_fetch_array($sql)){ ?>
		  <option><?php echo $r['ache'] ?></option>
		  <?php }?>
		  </select><input type="submit" value="Check for Drug" />	  
		</pre>
		</form>
          <div class="cleaner"></div>
        </div>
